...
MAP = [
    ...
    ("vehicles track marks", "units/vehicles/track_effects", "Decals.track", "WIRED",
     "per-planet tank tracks plus mirrored headings; jeep desert tracks"),
    ("vehicles tank_dirt/oil/sparks", "units/vehicles", "damage smoke/oil/sparks", "PARTIAL",
     "oil stains and directional smoke/sparks wired; tank_dirt placement is unknown"),
    # --- cannons ----------------------------------------------------------
    ...
    ("other/fire", "other/fire", "burning fire loops", "WIRED",
     "wreck fires"),
    ("other/particles, birds, hut_animals", "other", "ambient map life", "WIRED",
     "planet pools spawn birds and hut animals with their original calls"),
    # --- HUD --------------------------------------------------------------
    ("hud unit icons", "other/hud", "selection bar portraits", "WIRED",
     "icon_<type>_<team>.png (converted from bmp)"),
    ("hud unit labels", "other/hud", "selected-object name plates", "WIRED",
     "unit_label_<type>_<team> with neutral weapon-label fallback"),
    ("hud amount bar", "other/hud", "bottom territory gauges", "WIRED",
     "one team-coloured bar and controlled-zone count per active side"),
    ("hud grenade icon", "other/hud", "upgrade indicator (top bar)", "WIRED", ""),
    ("hud backdrop", "other/hud", "minimap radar frame per planet", "WIRED", ""),
    ("hud main_hud_side", "other/hud", "fixed side and bottom HUD chrome", "WIRED",
     "team sidebar, split bottom bar and resolution fillers"),
    ("hud a/b/d/g buttons", "other/hud", "HUD actions and modes", "WIRED",
     "A/T/D/Z and R/V/B/G/Menu use all three original states"),
    # --- menus / production GUI -------------------------------------------
    ("GOG Buttons.png", "gog/PNG/Buttons.png", "menu button plates (all states)", "WIRED (GOG)",
    ...
    ("GOG Box* menu chrome", "gog/PNG/Box*.png", "menu panel 3-slice", "WIRED (GOG)",
     "OriginalPanel; BoxInfo/BoxDivide spare"),
    ("GOG IP* multiplayer set", "gog/PNG/{IPBackground,IPSplash,MultiPlayer,IPBox*}.png",
     "multiplayer screens backdrop", "PARTIAL (GOG)",
     "IPBackground backs the MP browse + lobby screens; IPSplash/MultiPlayer.png atlas/2x IPBox* chrome spare"),
    ...
    ("GOG PMHSprites", "gog/PNG/PMHSprites.png", "visual reference", "REFERENCE ONLY",
     "duplicate port atlas; runtime uses cleaner extracted zod HUD pieces and unit icons"),
    ("GOG Font.png + Fonts/*.fnt", "gog/PNG/Font.png, gog/Fonts", "-", "COPIED (GOG), NOT WIRED",
     "Mac-port AHF bitmap font; superseded by zod per-char sets"),
    ("zod fonts", "zod/fonts/{yellow_menu,big_white,...}", "menu font (BMFont) via tools/zod/pack_fonts.py", "WIRED",
     "83 chars each; $ ( ) & # absent - engine fallback covers"),
    ("menus+main_menu_gui buttons", "other/menus, other/main_menu_gui", "UiTheme fallback chrome + list/popup art", "WIRED",
     "nine-piece styleboxes back the GOG plates; list_* skins ItemList/popups"),
    ...
    ("voices", "zod sounds + GOG lproj RAWs", "robot and commander voices", "WIRED",
     "selected/acknowledge lines mapped; ROB23-75 safely used as unlabelled chatter"),
    # --- GOG packed -------------------------------------------------------
