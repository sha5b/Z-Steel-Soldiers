...
func _texture_path(destroyed: bool) -> String:
	var suffix := "_destroyed" if destroyed else ""
	match String(ContentDB.building_def(building_id).get("tex", "")):
		"fort_front":
			return "res://assets/z/buildings/fort/fort_%s_front%s.png" % [planet, suffix]
		"fort_back":
			return "res://assets/z/buildings/fort/fort_%s_back%s.png" % [planet, suffix]
		"bridge":
			return "res://assets/z/planets/bridge_%s.png" % planet
		var kind:
			return "res://assets/z/buildings/%s/base_%s%s.png" % [kind, planet, suffix]
